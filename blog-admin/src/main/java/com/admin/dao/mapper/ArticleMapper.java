...
public interface ArticleMapper {
    @Select("select * from my_article where del_fag=0")
    ...
    @Select("select * from my_article where id=#{id} and del_fag=0")
    ...
    @Select("SELECT COUNT(*) from my_article where del_fag=0")
    int articleCount();
    ...
    @Update("update my_article set del_fag=1 where id=#{id}")
    int deleteArticle(Integer id);

    @Update("update my_comment set del_fag=1 where article_id=#{id}")
    ...
    @Update("update my_tag_article set del_fag=1 where article_id=#{id}")
    ...
    @Update("update my_tag_article set del_fag=1 where tag_id=#{id}")
